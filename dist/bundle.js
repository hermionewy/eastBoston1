...
/***/ (function(module, exports, __webpack_require__) {
	...
	var svg = d3.select("#circle-graphic").append("svg").attr('transform', 'translate(0,0)').attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom);
	var circleR = 3;
	if (width < 600) {
	    circleR = 2;
	}
	...
	function barByRaceX(d) {
	    var scaleX1 = d3.scaleBand().domain(axisData).range([0, width / 2 - 30]);
	
	    var scaleX2 = d3.scaleBand().domain(axisData).range([width / 2 + 30, width]);
	...
	function barByRaceX2(d) {
	    var scaleX = d3.scaleBand().domain(axisData).range([width / 7, 6 * width / 7]);
	...
	function forceLayout(data, forceX, forceY, colorRace, opacity) {
	    ...
	    simulation.force("collide", d3.forceCollide(circleR).radius(circleR).strength(1)).force("charge", d3.forceManyBody().strength(0.5)).restart().alpha(0.06).force('y', forceY).force('x', forceX).on('tick', ticked);
	
	    function ticked() {
	        d3.selectAll('.circleNode').attr('cx', function (d) {
	            return d.x;
	        }).attr('cy', function (d) {
	            return d.y;
	...
	function drawCircles(data) {
	    var node = svg.selectAll('.node').data([1]);
	
	    // Enter any new modes at the parent's previous position.
	    var nodeEnter = node.enter().append('g').attr('class', 'node').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
	
	    var circles = nodeEnter.merge(node).selectAll('.circleNode').data(data).enter();
	
	    var circlesUpdate = circles.append('circle').attr('class', 'circleNode');
	
	    d3.selectAll('.circleNode').attr('r', circleR);
